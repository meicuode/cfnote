...
- 文件管理批量操作（P13.3）：多选（`Set<number>` + Shift 连选 + 全选本页）后走**一个** `POST /api/fm/files/batch {op:'move'|'delete'|'copy', ids, folder_id, force}`，而不是前端循环打 N 次 `PUT`/`DELETE`——移动 20 个文件就是 20 次计费请求。删除沿用「仍被笔记引用需确认」的规矩，只是一次把名单全给出来（`needs_force` + `referenced`）。**复制**是唯一有真实成本的：R2 的 Workers binding 没有服务端 copy，必须 `get` 出来再 `put` 回去、字节流经 Worker，且存储真的翻倍，故限一次 ≤20 个、单个 ≤10MB，超出的跳过并原样报回；副本刻意不继承分享（分享是「这一份」的授权）。副本名由 `copyName()`（`src/lib/fmUtils.ts`，可单测）算出，反复复制加序号而不是叠「副本 副本」。拖拽移动：行 `draggable` + 载荷 `application/x-cfnote-files`，落点在 `FileManagerNav` 的每个目录节点（自定义 type 在 `dragover` 阶段读不到数据，只能靠 `dataTransfer.types` 判高亮，真正的 id 到 `drop` 才取）；落地经 `useFileManager.moveFilesToFolder`——拖起来的是右侧列表的行、落下的是侧栏的节点，两个组件只共享这个 hook。
