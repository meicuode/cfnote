...
- **P18.3，按待办选推送渠道**：三件事，第三件是主的。
  - **备注里贴代码块会把新建弹窗撑破。** 根因是 flex 子项默认 `min-width: auto`——里面有个不换行的 `<pre>` 时，子项被撑到内容宽度而不是出现滚动条，整个对话框跟着横向溢出。加一个 `.cfnote-md-narrow`（`min-width: 0` + `pre` 横向滚动 + 图片/表格限宽），并给编辑对话框的滚动区补 `min-w-0`。**这和 P17.3 侧栏那处 `min-h-0` 是同一类问题，只是换了个轴**——`flex-1` 的子项不给 `min-*: 0`，它就会撑破容器而不是滚动。图片和宽表格一并处理了，那是同样的撑破方式。
  - **默认提前量从 0 改成 30 分钟。** 默认 0 意味着「提醒」与「已经晚了」同时发生，那时已经来不及做任何事；30 分钟是能起身处理一件事的最小窗口，又不至于早到让人忘记。
  - **按待办选推送渠道。** 此前一条待办会发到哪里完全看不出来，而这个模块的重点就是通知。勾选框的选项来自当前已启用的渠道，默认全选。
  - **存 id 数组，而「全选」与「不选」都存 `null`**——表示「跟随全部已启用」而不是当时那几个的快照。这个区别只在以后加渠道时才显现：存快照的话新渠道不会自动纳入，而人不会回头去逐条勾选。`serializeChannels` 负责这个收敛（选中数 == 已启用数 → `null`），`parseChannels` 对坏值一律当「跟随全部」——宁可发给全部，也不要因为一个坏字段就静默不提醒。
  - **指定的渠道后来被停用/删除时，既不回落也不静默。** 回落是拿一个人没选的渠道去发他的私事，而他恰恰亲手指定过，说明他在意发到哪里；静默则是这个模块最坏的失败形态。所以推送侧写一条系统日志，同时那一行在面板上标红「指定渠道已失效」——让「这条发不出去」看得见。行内那个标签也刻意**只在指定过渠道时才显示**：「跟随全部」是默认，每行都挂一句「发给全部渠道」是噪音。
  - 周期任务滚出的下一条**继承渠道选择**：上一条指定发飞书，下一条不该悄悄变回发全部。
  - **`channels` 列走 `ALTER` 而不是只写进 `CREATE`。** `todos` 表在 P18 那批就建出来了，而 `CREATE TABLE IF NOT EXISTS` 对已存在的表是空操作——只写 `CREATE` 的话，「P18 那批就建好的库」永远没有这一列，而那正是我们自己的生产库。这是 P16.9.1 那个坑的变形：那次是漏了一处，这次是**建表语句本身对老库无效**。
