...
│   ├── lib/                   # 前端纯逻辑（markdown 渲染与渲染后增强、高亮补充语言表、URL 路由解析、窄屏返回栈层级、并发请求单飞闸门、博客文章 URL slug、博客布局与导航菜单、博客文章块部件、博客皮肤与主题库、博客自定义脚本解析、博客 SEO/预渲染字符串、博客列表筛选与标签云聚合、评论线程与待审本地合并、评论正文极小 Markdown 子集、通知渠道与凭据掩码、自动备份的到期与保留规则、剪藏、缩略图、敏感扫描、图片调宽、xmind、fm 工具等）
│   ├── hooks/
