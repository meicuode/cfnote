...
- **博客页面布局**：仿 WordPress 小工具——列表页与详情页**各一套**配置，把模块摆进顶部 / 左侧栏 / 右侧栏 / 底部四个槽位，可拖拽换位与排序、增删、停用、改标题与内容；侧栏宽度可调且滚动时固定，配置时实时提示正文剩余宽度，窄屏侧栏可选择并到顶部/底部而不是直接消失。内置模块：热榜、关于本站、自定义 Markdown、最新文章、标签云、友情链接、站内搜索。配置在「博客管理 → 页面布局」，存于设置表（无 schema 改动），默认配置与改造前完全一致
- **请求预算**：博客一个页面只打一次 API——布局、菜单、侧栏模块数据（热榜三档/最新文章/标签云）全部随该次响应下发，按当前页布局决定查哪几份，没启用的模块一行都不读；热榜切日/周/月零请求，评论区滚到附近才加载。列表页首屏 2 → 1 次请求，详情页 4 → 1 次
...
用 Vitest 覆盖后端 `worker/utils.ts` 中的纯函数（分块、JWT、密码哈希、内容哈希、think 标签清理、模型白名单、超时保护、AE 埋点结构）与前端纯逻辑（标签解析、任务勾选回写、私密文件夹判定与分享有效期、剪藏 bookmarklet 生成、URL 路由解析、评论校验与嵌套线程、待审评论本地合并、评论头像取色、章节锚点 slug、博客布局与导航菜单容错解析、博客列表筛选与 LIKE 转义与标签云聚合、高亮语言别名归一、敏感扫描、图片调宽等），共 27 个测试文件、292 个用例，无需任何 Cloudflare 环境即可运行。
