...
- **笔记本管理**：创建/删除笔记本，每个笔记本包含多篇文章；侧栏可折叠（状态记忆）。**重命名**有三个入口——右键菜单、双击笔记本名、选中后按 F2，就地变输入框，Enter 或失焦提交、Esc 取消；同级已有同名时给提示但不拦（导入按完整路径匹配，两本同级同名会分不清，但那是延迟的后果，不该在改名时变成硬性禁止）
...
- **URL 路由与刷新恢复**：当前笔记本/文章/主模块面板（文件管理·设置·统计·日志·博客管理·评论管理）编入地址栏，刷新与浏览器前进/后退可恢复视图；文件管理与博客管理为**内联工作区视图**（非弹窗，占据侧栏右侧区域），文件管理的导航（全部文件/未引用/笔记附件/我的文件夹）是侧栏二级菜单且当前所在文件夹也进 URL（`?panel=files&fm=folder:3`）；可把某篇笔记作为链接分享（`/nb/:id/:articleId`）。手写轻量路由，不引入 react-router
