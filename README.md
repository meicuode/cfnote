...
- **文件管理批量操作**：列表支持复选框多选（按住 Shift 连选一段、标题栏可全选本页），选中后一次性**移动 / 复制 / 删除 / 复制链接**；也可以直接**把选中的文件拖到左侧目录树**上移动（含「拖到此处移出文件夹」落点）。批量走一个 `POST /api/fm/files/batch`，不是前端循环打 N 次请求。删除时若有文件仍被笔记引用，服务端先回一份名单让你确认；复制会真的在 R2 多存一份（副本是新链接、不继承分享），因此限一次 ≤20 个、单个 ≤10MB，超出的跳过并列出原因
