...
- **博客页面布局**：仿 WordPress 小工具——列表页与详情页**各一套**配置，把模块摆进顶部 / 左侧栏 / 右侧栏 / 底部四个槽位，可拖拽换位与排序、增删、停用、改标题与内容；侧栏宽度可调且滚动时固定，配置时实时提示正文剩余宽度，窄屏侧栏可选择并到顶部/底部而不是直接消失。内置模块：热榜、关于本站、自定义 Markdown、最新文章、标签云、友情链接。配置在「博客管理 → 页面布局」，存于设置表（无 schema 改动），默认配置与改造前完全一致
...
用 Vitest 覆盖后端 `worker/utils.ts` 中的纯函数（分块、JWT、密码哈希、内容哈希、think 标签清理、模型白名单、超时保护、AE 埋点结构）与前端纯逻辑（标签解析、任务勾选回写、私密文件夹判定与分享有效期、剪藏 bookmarklet 生成、URL 路由解析、评论校验与嵌套线程、待审评论本地合并、评论头像取色、章节锚点 slug、博客布局容错解析与编辑、高亮语言别名归一、敏感扫描、图片调宽等），共 26 个测试文件、266 个用例，无需任何 Cloudflare 环境即可运行。
...
│   │   ├── BlogLayoutPanel.tsx # 博客页面布局配置（模块 × 顶部/左栏/右栏/底部槽位，拖拽排序，存 settings.blog_layout）
...
│   ├── lib/                   # 前端纯逻辑（markdown 渲染与渲染后增强、高亮补充语言表、URL 路由解析、评论线程与待审本地合并、剪藏、缩略图、敏感扫描、图片调宽、xmind、fm 工具等）
│   ├── hooks/
