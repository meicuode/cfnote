...
| 定时任务 | Cron Triggers，每月自动归档用量统计 + 回收站过期清理；每 5 分钟推送到期提醒 + 判定自动备份是否到期 |

## 核心功能

### 编辑与组织

- **笔记本管理**：创建/删除笔记本，每个笔记本包含多篇文章；侧栏可折叠（状态记忆）。**重命名**有三个入口——右键菜单、双击笔记本名、选中后按 F2，就地变输入框，Enter 或失焦提交、Esc 取消；同级已有同名时给提示但不拦（导入按完整路径匹配，两本同级同名会分不清，但那是延迟的后果，不该在改名时变成硬性禁止）。侧栏分两个滚动区：**笔记本树与标签自己滚，下面那几个固定入口（我的私有/回收站/博客管理/文件管理/网页剪藏）钉在底部**，笔记本再多也不会把它们顶到看不见
- **文件夹在主窗口里逛**：文件管理的侧栏只留四个固定入口，文件夹树在右侧主窗口按资源管理器的样子展示——文件夹行排在文件之前、双击进入、面包屑逐级跳回、一次只显示一层。新建/重命名/移动/删除走右键菜单（不再是行尾那几个悬浮小图标），把文件拖到文件夹行或面包屑上即移入，拖到「我的文件夹」那一级即移出。**「笔记附件」是平铺列表 + 按笔记本筛的 chips 而不是树**：同一个附件可能被多个笔记本的文章引用，树形会让它同时挂在两处、各本计数之和大于文件总数；列表里它只出现一次，行内显示被几篇笔记引用，点开看具体是哪几篇
- **三模式编辑**：源码 Markdown / 富文本所见即所得 / 预览，一键切换，3秒无操作自动保存；全程标准 Markdown 存储（无私有方言，可自由迁移）
- **保存不会存重**：新建笔记时手动保存与自动保存撞在一起也只会建出一篇（同一篇草稿的创建请求全局单飞）；保存失败会在状态栏说明并自动重试三次，之后转人工重试，不会在离线时无限打接口
- **富文本编辑器**：基于 Tiptap，支持标题/列表/引用/代码块/表格/图片等，粘贴网页富文本自动转 Markdown，粘贴代码/Markdown 原样保留
- **代码高亮 / 数学公式 / 图表**：预览与博客对代码块做语法高亮（highlight.js，常用 36 种内置 + PowerShell/Dockerfile/Nginx/LaTeX 等 31 种按需加载，别名如 `ps1`/`docker`/`bat` 自动归一，冷门语言退回自动检测且不再报警告）、渲染 `$…$` / `$$…$$` LaTeX 公式（KaTeX）、把 ` ```mermaid ` 代码块渲染为流程图/时序图等（mermaid）；三库均按需懒加载，无对应内容的页面零开销
- **附件与图片**：拖入/粘贴截图直传 R2，任意文件上传；XMind 文件在线预览+编辑回存；编辑器可从文件库选择已有附件插入
- **文件管理多选与批量操作**：列表按 Windows 资源管理器的方式选择——**单击选中、Ctrl/⌘ 点选、Shift 连选、Ctrl+Shift 追加一段、Ctrl/⌘+A 全选、Esc 清空、双击打开**；**右键出菜单**（打开/下载、复制链接、分享、重命名、移动到、复制到、查看引用、删除；右键未选中的行会先选中它，右键选中集里的行则整段保持，菜单作用在整个选择上；空白处右键是上传/全选/刷新）。选中后一次性**移动 / 复制 / 删除 / 复制链接**，也可以直接**把选中的文件拖到左侧目录树**上移动（含只在拖拽时出现的「拖到此处移出文件夹」落点）。**底部状态栏**没选中时显示当前目录合计（文件数 + 总大小），选中后显示「已选 N 个 · 共 X」。行首复选框默认不显示，**触屏设备自动显示**（那里没有修饰键），可在「设置 → 文件管理」改成总是显示/从不显示（存本机，文件管理右上角齿轮可直达）。批量走一个 `POST /api/fm/files/batch`，不是前端循环打 N 次请求。删除时若有文件仍被笔记引用，服务端先回一份名单让你确认；复制会真的在 R2 多存一份（副本是新链接、不继承分享），因此限一次 ≤20 个、单个 ≤10MB，超出的跳过并列出原因
- **标签**：文章标题下打标签（Enter/逗号添加，datalist 补全已有标签，上限 20）；侧栏标签区可折叠、按使用频次排序显示常用标签为紧凑 chips，标签多时点「全部标签」开可搜索浏览器；列表项显示前 3 个
- **置顶**：列表悬浮 📌 切换，置顶排最前
- **回收站**：删除进回收站（软删除），30 天内可恢复（重建向量索引）或彻底删除，30 天后自动清理。**删整本笔记本同样只是进回收站**（此前是彻底删除，连 R2 上的附件都当场清掉），可「恢复整本」把其中的笔记一并带回；恢复单篇时若原笔记本也在回收站里，会连带把它一起恢复。**附件不随软删除消失**——回收站里的笔记算活着的引用，只有彻底删除时才按「引用归零 + 未归入文件管理的文件夹」清理 R2；点「清空回收站」会先告诉你这次将连带清掉几个附件、共多大
...
- **深色模式**：跟随系统或手动切换（记忆）
- **手机上能看**：窄屏（<1024px）不再是把四栏硬塞进 390px，而是一次显示一层——侧栏 → 文章列表 → 正文，顶栏出 ← 逐层返回，刷新与浏览器前进/后退仍停在原来那一层。编辑器在手机上**默认进预览**（要写点「源码」，富文本在移动端仍是只读）；AI 对话变全屏覆盖层；顶栏放不下的入口（主题/统计/设置/日志/退出）收进「⋯」菜单。触屏没有「鼠标悬浮」，所以文章列表的置顶/删除、博客管理行的预览/取消公开改为常驻显示，文件管理行尾给一个「⋯」直接开右键菜单。**桌面布局一个像素都没动**——全部改动只在窄屏断点里生效。定位是「只看」：没做离线、没做 PWA，断网就是打不开
...
- **本地导入**：选文件或整个文件夹（递归读取 .md / .markdown / .txt）。默认**保留文件夹结构**——子目录建成对应的子笔记本，同名的直接复用；取消勾选则平铺进当前笔记本、标题带上相对路径。大文件夹自动分片上传并显示真实进度，导入后自动分批建立向量索引
- **深/浅视图**：点笔记本默认只看这一本，勾「含子级」连子孙笔记本的文章一起看，每条标出归属路径
- **私密审计**：「我的私有」列出全部私有笔记及其归属，区分继承来的与单篇显式标的；若有笔记落在私密笔记本里却没上锁，顶部红条列出是哪几篇、哪些还公开在博客上
- **JWT 吊销与改密码**：改密码会立刻吊销所有设备上的旧登录，接口直接返回新凭据无需重新登录；每次鉴权多一次单行主键查询（额度够用：请求数 10 万/天是瓶颈，epoch 查询只吃掉行读的 2%）
- **网页剪藏**：书签栏 bookmarklet 抓取选区/正文 → `/clip` 页 turndown 转 Markdown → 选笔记本存为笔记
- **公开博客**：一键公开笔记到整站博客 `/blog`（IT之家风格，亮/暗双主题，浏览计数去重，热榜）；发布前对全文做敏感信息检查；长文详情页左侧有**可展开的章节目录**（默认收起，≥3 个标题才出现，点章节跳转并把 `#章节` 写进地址栏，可复制分享到某一节）
- **列表分页与标签筛选**：列表每页 20 篇 + 「加载更多」（此前是硬上限 100 篇且无分页）；标签云、列表行 Tags、详情页 Tags 与面包屑**全部可点**，点了按该标签/笔记本筛选（`/blog?tag=xxx`，进地址栏可复制可后退），另有站内搜索模块（`/blog?q=关键词`，匹配标题与正文）。筛选与分页都在服务端完成，能在全部公开文章里找，不受「本地只加载了前几页」的限制
- **博客导航菜单**：顶栏菜单可配置（仿 WordPress「外观 → 菜单」），菜单项可以是首页 / 某个标签或笔记本 / 某篇公开笔记（通常指向一篇**单页**，比如「关于我」）/ 外部链接；窄屏自动收进汉堡按钮。配置随页面布局一起下发，不额外占用请求
- **博客页面布局**：仿 WordPress 小工具——列表页 / 详情页 / **单页各一套**配置，把模块摆进顶部 / 左侧栏 / 右侧栏 / 底部四个槽位，可拖拽换位与排序、增删、停用、改标题与内容；侧栏宽度可调且滚动时固定，配置时实时提示正文剩余宽度，窄屏侧栏可选择并到顶部/底部而不是直接消失。内置模块：热榜、关于本站、自定义 Markdown、最新文章、标签云、友情链接、站内搜索、**幻灯片/焦点图、站点横幅、上一篇/下一篇、相关文章、文章宫格**（后五种对标 WordPress 主题的 header/footer 组件，添加菜单按槽位分「常用/其他」）。幻灯片有三种形态：单图铺满（侧栏用）、轮播露边、**主图 + 侧栏标题**（后两种适合顶部通栏，按宽高比排版而不是固定像素高——单图铺满在通栏里会被拉得又宽又扁）。配置在「博客管理 → 页面布局」，存于设置表（无 schema 改动），默认配置与改造前完全一致
- **所见即所得地调布局**：配置页右侧是**真的博客页**（`/blog?preview=1` 装进 iframe），改一下模块立刻看到效果——不是示意图，就是那个页面本身。仿 WordPress 自定义器：改动靠 postMessage 即时下发不重载，点预览里的模块可在左侧编辑它；iframe 按 1400/1000px 真实宽度渲染再整体缩放，所以「窄屏」预览触发的是真的 CSS 断点；预览不计浏览量
- **博客主题**：「博客管理 → 页面布局 → 主题外观」可换配色与排版——5 套预设（IT之家红/墨绿/深蓝/酒紫/石墨）、主色与顶栏色取色器（悬浮色可跟随主色自动提亮）、圆角/正文字号/容器宽度滑块、字体（无衬线/衬线/等宽）、列表样式（左图右文 / **纯文字列表**，后者不加载缩略图，省掉每篇一次图片请求），以及**额外 CSS**（等价 WordPress 的「额外 CSS」，只作用于博客页，可覆盖任意 `--blog-*` 变量）。改动在右侧预览里实时可见；默认主题与改造前完全一致
- **主题库**：调顺手的整套配置可以**存下来命名**，存多套一键切换（内置那 5 个预设只是「一键改两个颜色」，不是主题）；在「博客管理 → 页面布局 → **主题库**」独立页签里管理——套用、把当前配置写回某套、**复制一份**、重命名、删除（二次确认），以及**导出/导入 JSON**。导出的是一份配置（配色 / 排版 / 额外 CSS），不是 WordPress 那种含模板的主题包，**也不含任何 JS**。导入他人主题时会剥掉额外 CSS 里的 `@import` 并列出其中的外部地址——CSS 不能执行 JS，但能把访客的 IP 与来源发给第三方。主题库存在单独的设置键里，**博客页一行都不读它**，所以存多少套都不影响访客的加载
- **单页**：对标 WordPress 的**文章 vs 页面**——把一篇已公开的笔记设为「单页」（如「关于我」「公告」），它仍然可通过菜单和链接访问、也仍然进 sitemap，但**不再出现在列表页、热榜、相关文章、上一篇/下一篇与 RSS 里**。此前菜单早就能指向某篇公开笔记，缺的从来不是入口而是「不进文章流」的能力。单页有**自己的一套槽位布局与文章块部件**（默认只留标题 + 正文——「关于我」不该带发布时间和浏览数），并且是唯一允许整块关掉评论区的地方（全局开关仍是唯一决定能否提交的地方，这里只是不渲染）。URL 保持 `/blog/:id` 不变，预渲染、边缘缓存、真 404 全部原样复用。管理入口在「博客管理 → 已公开文章」的 `全部 / 文章 / 单页` 筛选与行内「设为单页」
- **文章块可配置**：详情页正文区不再写死——面包屑 / 标题 / 元信息（时间、来源、Tags、浏览数各自可开关，来源文字可改）/ 正文 / 标签行 / 结束标记（文字可改）/ 版权声明（按 Markdown 渲染）/ 评论区，可排序、可开关。这是那四个槽位管不到的部分（它们管的是**文章周围**），对标 WordPress 主题的 `single.php`。**不做模板语言**：部件是一份固定成员的声明式清单，只能排序与开关，这样服务端预渲染照同一份配置产出 HTML，抓取器看到的顺序与读者一致；正文与评论区锁定为启用（评论的真开关在「设置 → 评论」；单页那套除外）。配置在「博客管理 → 页面布局 → 详情页 / 单页」，默认与改造前逐项一致
- **请求预算**：博客一个页面只打一次 API——布局、菜单、侧栏与顶底模块要的数据（热榜三档/最新文章/标签云/幻灯片/宫格/相关文章/上下篇）全部随该次响应下发，按当前页布局决定查哪几份，没启用的模块一行都不读；热榜切日/周/月零请求，评论区滚到附近才加载。列表页首屏 2 → 1 次请求，详情页 4 → 1 次。加了五个新模块之后请求数仍然不变，图片是唯一的增量成本（幻灯片只渲染当前±1 张，宫格在页面底部天然 lazy）
- **搜索引擎与分享卡片**：详情页 HTML 由 Worker **服务端预渲染**——标题、摘要、配图、canonical、og/twitter 卡片与 JSON-LD 结构化数据进 `<head>`，正文与「菜单/上下篇/相关文章/标签」的内链进 `<body>`。这是必要的：微信、微博、Twitter、Slack 的链接预览抓取器与百度蜘蛛都**不执行 JS**，凡是异步取回的内容它们一律看不到（此前每篇文章分享出去都是同一张「CFNote - 私人知识库」空卡片）。同时提供 `/robots.txt`、`/sitemap.xml`（「加载更多」的必要配套：第 21 篇之后抓取器只能靠它发现）与 `/blog/feed.xml`（RSS）。文章不存在时返回真 404 而不是 soft-404。**这不是 React SSR**——客户端仍是 `createRoot` 重画一遍而非 `hydrateRoot` 接管，服务端那份是给抓取器和首屏文字用的过渡态；详见 `docs/public-blog.md`
- **预渲染开关**：「设置 → 博客详情页预渲染」三档——完整预渲染（默认，正文与状态都内联，前端不再拉接口，每次访问 1 次 Worker 请求）/ 仅 meta（正文仍由前端拉，2 次请求）/ 关闭（原样透传，2 次请求）。注意三档都会经过 Worker（由 `wrangler.toml` 的 `run_worker_first` 决定，运行时改不了），所以「关闭」并不更省——它是出问题时**不必重新部署就能恢复**的开关
- **博客管理**：侧栏「博客管理」内联展示（非弹窗）所有已公开文章，**按修改时间降序**，支持标题搜索、按笔记本过滤、预览、一键取消公开；**左列表 + 右编辑器两栏**，点选即可就地编辑正文（源码/富文本/预览三模式，与笔记编辑器完全一致），保存后列表不重排、位置不跳（`?panel=blog` 可刷新保持）；其下「评论管理」为二级菜单（`?panel=comments`）
- **评论**：公开博客文章底部支持访客评论（昵称必填、邮箱可选不公开），**默认需审核**（设置可切免审核），支持 2 层嵌套回复与博主回复；正文支持**表情面板**与一小撮 Markdown——`**粗体**`、`*斜体*`、`~~删除线~~`、`` `行内代码` ``、`> 引用`、`- 列表`、`[文字](链接)` 与裸链接自动成链；**刻意不给**标题、图片、表格、代码块与裸 HTML（评论区不是文章），链接只放 http/https、一律 `nofollow`，且一条评论最多 3 个链接、超出整条按纯文本显示（防链接刷屏）。渲染走自写的 token 解析 + 白名单 React 元素，**永不经过 marked、永不 `dangerouslySetInnerHTML`**；评论人名前有**头像占位**（昵称首字 + 按昵称确定性取色，不走 Gravatar 因而不外泄邮箱、无外部请求）；提交后**自己那条评论会立刻浅色显示并标记「待审核」**（刷新仍在，通过后自动恢复正常样式），不必担心内容丢了；侧栏「博客管理 → 评论管理」审核（通过/拒绝/删除/回复，鼠标悬浮高亮当前行、正在回复的那条常驻高亮），待审计数徽标，「查看↗」带评论锚点可直达该楼，列表**保持纯文本显示**（审核要看访客写的原始字符）并显示提交时的 **IP 与浏览器 UA**（仅博主可见，公开接口从不返回）；轻量防刷（每 IP 每分钟限流 + 蜜罐 + 长度上限），有待审评论可推送到已配置的通知渠道
- **博客自定义脚本**：「设置 → 博客自定义脚本」可粘一段统计代码/客服挂件——直接粘服务商给的 `<script src=…>` 片段即可（大多数人手里的就是 HTML 片段而不是纯 JS），也支持写纯 JS。**客户端注入、单一路径**：列表页与详情页都覆盖，与预渲染档位完全解耦（那个开关是 kill switch，不该把统计代码一起关掉），随博客响应下发因此零新增请求，整个 SPA 生命周期只执行一次。四种情况一律不注入且不给开关：管理端页面、布局预览 `?preview=1`、私密分享页、以及带 `?nojs=1` 打开时（逃生阀，脚本写崩了用它进博客页再回后台改）。**风险讲在明处**：这段代码与管理端、`/api/*` 同源，能读到登录凭据——危险的不是你自己写的那几行，是从别处粘来的第三方脚本；主题的导入导出永远不携带它
- **可读的文章地址**：`/blog/12` → `/blog/12/部署-cloudflare-workers`，slug 由标题现算（中文原样保留）。**id 在前、slug 只是装饰**——查表永远只认 id，所以改标题不会把老链接变成 404，`/blog/12` 也照常打得开；站内所有链接（列表、热榜、相关、上下篇、sitemap、RSS、管理端「预览↗」）一律直发带 slug 的地址，`<link rel="canonical">` 指向它。不存库、不加列、不做重定向
...
- **URL 路由与刷新恢复**：当前笔记本/文章/主模块面板（文件管理·设置·统计·日志·博客管理·评论管理）编入地址栏，刷新与浏览器前进/后退可恢复视图；文件管理与博客管理为**内联工作区视图**（非弹窗，占据侧栏右侧区域），文件管理的导航（全部文件/未引用/笔记附件/我的文件夹）是侧栏二级菜单，当前所在文件夹与按笔记本筛的选择也进 URL（`?panel=files&fm=folder:3`、根层为 `fm=folder`）；可把某篇笔记作为链接分享（`/nb/:id/:articleId`）。手写轻量路由，不引入 react-router
- **首次初始化引导**：自动检测系统状态，引导创建数据库和用户
- **登录爆破防护**：`/api/auth/login` 每 IP 每 15 分钟至多 8 次失败，超限返回 429 + `Retry-After`，且**限流判断排在密码校验之前**——每次校验是 10 万轮 PBKDF2，不拦住的话一行 `curl` 循环就能持续烧 CPU 而根本不用猜对密码；登录成功清账。用户不存在时也照算一遍等价哈希再返回同一句错误，避免从响应时间读出「用户名对不对」。计数走 Cache API（零配额、按数据中心生效），缓存不可用时**降级为放行**——单用户系统里把主人锁在门外没有第二条补救路径。它省的是 CPU 与 D1 读而**不是请求数**（请求进到 Worker 时已经计了），要在那一层挡请用 Cloudflare 的 WAF 速率限制规则
- **数据导出/导入**：一键导出全部笔记本、文章（连同**公开状态、单页标记、发布时间与浏览数**）、**访客评论**、标签、对话与**博客主题/布局配置**为 JSON，ZIP 完整备份另含全部附件；文章历史版本可勾选带上（体积会翻几倍，默认不带）。敏感项不导出（API Key、通知渠道凭据）。导入是合并式的：同名笔记本复用、重复文章跳过，评论跟着它所属的文章恢复（含楼中楼父子关系的重映射），博客配置**只在当前没有该项时才恢复**、不会冲掉现有设置；还支持本地 md/txt 批量导入
- **凭据不回显**：已保存的 API Key、Telegram Bot Token、Server酱 SENDKEY、飞书/钉钉加签密钥，以及企业微信/钉钉的 **Webhook 地址**（那串 URL 本身就是能往群里发消息的凭据）在设置面板只显示「已设置 ****后四位」，点「修改」才能换成新值——接口返回的就是掩码，不是前端遮一下
- **改密码与吊销**：「设置 → 账号」改密码，会把**所有设备上的登录立刻失效**（`users.token_epoch` +1，签发时写进 token、鉴权时比对），当前这台自动换上新凭据。怀疑 token 泄露时改一次密码就能全部断掉，不必等 7 天有效期
- **恢复码（忘记密码）**：登出了又忘了密码时唯一的自助出路。注册时给一个 128 bit 的码（`a3f9c1e0-8b7d4526-…`），登录页点「忘记密码」填它 + 新密码即可重置；重置会踢掉所有设备，码本身**一次性**、用掉就换一张。「设置 → 账号」里随时可看可复制可重新生成。它**明文存在 `users` 表上**——要能被人读出来抄下来，哈希了就没有意义；存在 users 表而不是 settings 里，是因为备份导出压根不含 users 表，所以它结构性地不会随备份文件流出去。挡刷靠 IP 限流（15 分钟 5 次），**没有「错 N 次作废」**：那会让攻击者随手猜几次就废掉你唯一的兑回路径，而 128 bit 的熵本来就穷举不动
- **自动备份到 R2**：手动导出要手点，手不点就没有备份。设置里选个频率（关闭 / 每 5 小时 / 每天 / 每 7 天）+ 保留份数，之后每次备份自动写进 R2 的 `backups/` 前缀并清掉超出份数的旧的，面板里可下载 / 一键恢复 / 删除，另有「立即备份一次」。**只备数据库**（笔记、评论、设置、附件清单），不含附件字节——附件本来就在 R2 里，复制一份只是把同一份风险买两遍；要连附件一起请用 ZIP 完整备份。失败会在设置面板红字显示原因并落进系统日志。备份文件只有本人能取，不会出现在文件管理里
...
点击顶栏右侧的齿轮图标打开设置面板。左侧是分类导航，右侧是该类的具体设置：

| 分类 | 内容 |
|------|------|
| 🤖 AI 对话 | LLM 模型选择、Jina API Key |
| 📝 博客 | 详情页预渲染档位、自定义脚本 |
| 💬 评论 | 评论开关、免审核开关 |
| 📁 文件 | 文件列表的多选方式（复选框显示策略） |
| 🔔 通知 | 提醒到期的推送渠道（企业微信 / 飞书 / 钉钉 / Server酱 / Telegram / 自定义 Webhook） |
| 💾 备份 | 导出 JSON / ZIP、导入备份、自动备份到 R2 |
| 👤 账号 | 修改密码（会吊销所有设备上的登录）、恢复码 |

底部的「保存」对所有分类一起生效——`PUT /api/settings` 本来就是整份提交，分类只是显示上的分组。「文件」那一类存 localStorage，改完立即生效，不用保存。

手机上先看到分类列表，点进去显示那一类，顶栏出返回箭头（Esc 也是先退回列表再关面板）。
...
**用恢复码（推荐）**：注册时给过你一个 128 bit 的恢复码（`a3f9c1e0-8b7d4526-…`），登录页点「忘记密码」就能用它重置。如果还能在某台设备上登录，去「设置 → 账号」可以看到当前这个码；都登不上了就得翻当初抄下来的那张纸（或密码管理器里的记录）。重置会踢掉所有设备，码本身一次性、用掉自动换一张，记得回设置里抄新的。

**直接改数据库（兜底，但有陷阱）**：连恢复码也没有的话，只剩这条路。注意**能读 D1 控制台不等于能改密码**——`password_hash` 是 PBKDF2-SHA256 十万轮，控制台里算不出来，所以没法直接 `UPDATE` 一个新密码进去。唯一的办法是删掉 `users` 那一行再回站点重新注册。

**但只写 `DELETE FROM users` 会让你以为笔记全丢了**：`users.id` 是 `AUTOINCREMENT`，SQLite 在 `sqlite_sequence` 里记着用过的最大值，重注册时新用户拿到 `id=2`，而所有笔记还挂在 `user_id=1` 上。正确的两句：

```sql
DELETE FROM users;
DELETE FROM sqlite_sequence WHERE name = 'users';   -- 少这句 = 笔记全「消失」
```

再回站点重新注册，拿回 `id=1`，数据全在。**注册完立刻去设置里改一次密码**——重注册后 `token_epoch` 归 0，旧的泄露 token（epoch 0、同用户名）会复活。

恢复码补的正是这一段：不必记这条 SQL、不会踩 `sqlite_sequence` 的坑、也不会复活旧 token。
...
**单元测试（41 个文件，无需任何 Cloudflare 环境）** —— 覆盖后端 `worker/utils.ts` 中的纯函数（分块、JWT、密码哈希、内容哈希、think 标签清理、模型白名单、超时保护、AE 埋点结构）与前端纯逻辑（标签解析、任务勾选回写、私密文件夹判定与分享有效期、剪藏 bookmarklet 生成、URL 路由解析、评论校验与嵌套线程、评论正文的极小 Markdown 子集与链接协议白名单、待审评论本地合并、评论头像取色、章节锚点 slug、博客布局与导航菜单容错解析、文章块部件与单页部件、博客皮肤解析与颜色派生与 CSS 清洗、博客列表筛选与 LIKE 转义与标签云聚合与相关文章打分、自定义脚本解析、通知渠道凭据掩码、文件副本命名与多选语义与右键菜单落点、高亮语言别名归一、敏感扫描、图片调宽、笔记本树与私密继承、限流窗口与坏记录解析等）。其中 `tests/schema.test.ts` 是一道静态锁：按文本比对 `system.ts` 的 `SCHEMA` 与 `migrate.ts` 的幂等语句，防止「migrate 里加了列、全新建表忘了加」这类漏改——那种漏改不会立刻报错，因为 `migrate` 的失败在 `worker/index.ts` 里是被吞掉的。

**Worker 端到端（10 个文件）** —— 由 `@cloudflare/vitest-pool-workers` 在 workerd（miniflare）里跑真的 `worker/index.ts`，配真的 D1 与 R2，用 `SELF.fetch()` 打接口。覆盖：全新初始化（`/api/init` 建表、逐列断言、可重复执行、`ensureSchema` 在全新库上不撞车）、老库幂等迁移（手工建早期 `articles`/`notebooks` 表 → 补齐全部新列且老数据不丢）、单页不进文章流（列表/RSS 排除、详情与 sitemap 保留、标记可逆且不被误清）、文件管理批量操作（真的往 R2 写副本、删对象、引用清单确认、越权 id 打不动）、备份导出导入往返（公开状态与单页标记、评论父子关系、设置白名单、重复导入不翻倍）、**回收站语义**（删笔记本只是软删且 R2 一个字节不动、整本恢复、恢复单篇连带恢复笔记本、附件三档判定、清空前预检与实际清理一致、30 天到期清理不误伤已恢复的）、**私密笔记本不变式**（整支强制上锁、软继承、回收站的两条恢复路径都过闸门、祖先在回收站里时链不断）、**登录限流**（超限连正确口令也一起挡住 = 证明它拦在校验之前、按 IP 分桶、成功清账、无 IP 头时放行）。配置见 `wrangler.test.toml`：**只声明 D1 + R2**，不声明 Vectorize/AI（无本地实现）也不声明 `[assets]`（否则测试就依赖 `dist/` 构建产物）。

类型检查有两份配置：`npm run build` 用 `tsconfig.json`（只含 `src`/`worker`），`npm run typecheck` 用 `tsconfig.tests.json` 额外纳入 `tests/`。分开是因为生产构建不该依赖测试文件能否编译，而测试目录长期不被 `tsc` 覆盖又确实漏过 bug。e2e 的绑定类型在 `tests/env.d.ts` 里声明（0.19 起 `env` 的类型是 `Cloudflare.Env`，不是旧文档里的 `ProvidedEnv`）。
...
│   ├── utils.ts                # 工具函数（JWT/哈希/分块/模型/AE/Jina）+ recountNotebook（article_count 唯一写法）
│   ├── rateLimit.ts            # 计数式限流的 Cache API 后端（登录与评论共用，纯逻辑在 src/lib/rateLimit.ts）
│   ├── migrate.ts              # 应用内幂等 schema 保障（增量加列/建表，与 system.ts 同步）
│   ├── archive.ts              # 用量归档（POST /api/stats/archive 与月度 Cron 共用）
│   ├── backup.ts               # 数据快照构建（/api/export 与自动备份共用）+ R2 自动备份调度
│   ├── repo/
...
│       ├── backups.ts          # /api/backups/*（自动备份列表/立即备份/下载/删除）
│       └── stats.ts            # /api/stats（统计仪表盘）、/api/stats/archive
├── src/                        # 前端 React SPA
│   ├── components/
│   │   ├── SetupPage.tsx      # 初始化 + 注册引导
│   │   ├── LoginPage.tsx      # 登录页
│   │   ├── Layout.tsx         # 主布局：桌面四栏（含 AI 面板，列宽可拖拽/侧栏可折叠），窄屏单列返回栈
...
│   │   ├── FileManager.tsx    # 文件管理右侧列表（搜索/预览/分享/清理/多选批量操作/右键菜单/拖拽到目录，内联工作区视图）
│   │   ├── FileManagerNav.tsx # 文件管理二级菜单（侧栏：全部/未引用/笔记附件/文件夹树+增删改，文件夹是拖拽放置目标）
│   │   ├── BlogManager.tsx    # 博客管理（已公开文章两栏内联编辑 + 文章/单页筛选 + 评论审核 + 页面布局，内联工作区视图）
│   │   ├── BlogLayoutPanel.tsx # 博客页面布局配置（模块 × 顶部/左栏/右栏/底部槽位，拖拽排序，导航菜单，列表页/详情页/单页各一套，主题外观 + 主题库，存 settings.blog_layout / blog_skin / blog_skin_library）
│   │   ├── BlogPreview.tsx    # 布局配置页右侧的实时预览（真博客页装进 iframe，postMessage 下发未保存的布局）
...
│   ├── lib/                   # 前端纯逻辑（markdown 渲染与渲染后增强、高亮补充语言表、URL 路由解析、窄屏返回栈层级、并发请求单飞闸门、本地导入的标题推导、博客文章 URL slug、博客布局与导航菜单、博客文章块部件、博客皮肤与主题库、博客自定义脚本解析、博客 SEO/预渲染字符串、博客列表筛选与标签云聚合、评论线程与待审本地合并、评论正文极小 Markdown 子集、通知渠道与凭据掩码、自动备份的到期与保留规则、剪藏、缩略图、敏感扫描、图片调宽、xmind、fm 工具等）
│   ├── hooks/
│   │   ├── useAuth.ts         # 登录状态 + 附件 cookie 副本
│   │   ├── useFileManager.ts  # 文件管理共享数据（overview + 文件夹增删改，侧栏与列表同源）
│   │   └── useApi.ts          # API 请求封装
...
├── tsconfig.json               # 构建用（只含 src/worker）
├── tsconfig.tests.json         # npm run typecheck：额外纳入 tests/
└── package.json
```
