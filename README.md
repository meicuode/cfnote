...
- **博客导航菜单**：顶栏菜单可配置（仿 WordPress「外观 → 菜单」），菜单项可以是首页 / 某个标签或笔记本 / 某篇公开笔记（当单页用，比如「关于我」）/ 外部链接；窄屏自动收进汉堡按钮。配置随页面布局一起下发，不额外占用请求
- **博客页面布局**：仿 WordPress 小工具——列表页与详情页**各一套**配置，把模块摆进顶部 / 左侧栏 / 右侧栏 / 底部四个槽位，可拖拽换位与排序、增删、停用、改标题与内容；侧栏宽度可调且滚动时固定，配置时实时提示正文剩余宽度，窄屏侧栏可选择并到顶部/底部而不是直接消失。内置模块：热榜、关于本站、自定义 Markdown、最新文章、标签云、友情链接、站内搜索、**幻灯片/焦点图、站点横幅、上一篇/下一篇、相关文章、文章宫格**（后五种对标 WordPress 主题的 header/footer 组件，添加菜单按槽位分「常用/其他」）。配置在「博客管理 → 页面布局」，存于设置表（无 schema 改动），默认配置与改造前完全一致
...
- **文章块可配置**：详情页正文区不再写死——面包屑 / 标题 / 元信息（时间、来源、Tags、浏览数各自可开关，来源文字可改）/ 正文 / 标签行 / 结束标记（文字可改）/ 版权声明（按 Markdown 渲染）/ 评论区，可排序、可开关。这是那四个槽位管不到的部分（它们管的是**文章周围**），对标 WordPress 主题的 `single.php`。**不做模板语言**：部件是一份固定成员的声明式清单，只能排序与开关，这样服务端预渲染照同一份配置产出 HTML，抓取器看到的顺序与读者一致；正文与评论区锁定为启用（评论的真开关在「设置 → 评论」）。配置在「博客管理 → 页面布局 → 详情页」，默认与改造前逐项一致
...
- **数据导出/导入**：一键导出全部笔记本、文章（连同**公开状态、发布时间与浏览数**）、**访客评论**、标签、对话与**博客主题/布局配置**为 JSON，ZIP 完整备份另含全部附件；文章历史版本可勾选带上（体积会翻几倍，默认不带）。敏感项不导出（API Key、通知渠道凭据）。导入是合并式的：同名笔记本复用、重复文章跳过，评论跟着它所属的文章恢复（含楼中楼父子关系的重映射），博客配置**只在当前没有该项时才恢复**、不会冲掉现有设置；还支持本地 md/txt 批量导入
...
│   │   ├── FileManager.tsx    # 文件管理右侧列表（搜索/预览/分享/清理，内联工作区视图）
│   │   ├── FileManagerNav.tsx # 文件管理二级菜单（侧栏：全部/未引用/笔记附件/文件夹树+增删改）
│   │   ├── BlogManager.tsx    # 博客管理（已公开文章两栏内联编辑 + 评论审核 + 页面布局，内联工作区视图）
│   │   ├── BlogLayoutPanel.tsx # 博客页面布局配置（模块 × 顶部/左栏/右栏/底部槽位，拖拽排序，导航菜单，主题外观 + 主题库，存 settings.blog_layout / blog_skin / blog_skin_library）
