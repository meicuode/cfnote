...
- **URL 路由与刷新恢复**：当前笔记本/文章/主模块面板（文件管理·设置·统计·日志·博客管理·评论管理）编入地址栏，刷新与浏览器前进/后退可恢复视图；文件管理与博客管理为**内联工作区视图**（非弹窗，占据侧栏右侧区域）；可把某篇笔记作为链接分享（`/nb/:id/:articleId`）。手写轻量路由，不引入 react-router
