...
- **主题库**：调顺手的整套配置可以**存下来命名**，存多套一键切换（内置那 5 个预设只是「一键改两个颜色」，不是主题）；支持重命名、把当前配置写回某套主题、删除，以及**导出/导入 JSON**。导出的是一份配置（配色 / 排版 / 额外 CSS），不是 WordPress 那种含模板的主题包。导入他人主题时会剥掉额外 CSS 里的 `@import` 并列出其中的外部地址——CSS 不能执行 JS，但能把访客的 IP 与来源发给第三方。主题库存在单独的设置键里，**博客页一行都不读它**，所以存多少套都不影响访客的加载
...
│   ├── lib/                   # 前端纯逻辑（markdown 渲染与渲染后增强、高亮补充语言表、URL 路由解析、博客布局与导航菜单、博客文章块部件、博客皮肤与主题库、博客 SEO/预渲染字符串、博客列表筛选与标签云聚合、评论线程与待审本地合并、剪藏、缩略图、敏感扫描、图片调宽、xmind、fm 工具等）
│   ├── hooks/
