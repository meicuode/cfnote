...
- **评论**：公开博客文章底部支持访客评论（昵称必填、邮箱可选不公开），**默认需审核**（设置可切免审核），支持 2 层嵌套回复与博主回复；评论人名前有**头像占位**（昵称首字 + 按昵称确定性取色，不走 Gravatar 因而不外泄邮箱、无外部请求）；提交后**自己那条评论会立刻浅色显示并标记「待审核」**（刷新仍在，通过后自动恢复正常样式），不必担心内容丢了；侧栏「博客管理 → 评论管理」审核（通过/拒绝/删除/回复），待审计数徽标，「查看↗」带评论锚点可直达该楼，列表显示提交时的 **IP 与浏览器 UA**（仅博主可见，公开接口从不返回）；轻量防刷（每 IP 每分钟限流 + 蜜罐 + 长度上限），有待审评论可推送到已配置的通知渠道
...
│   ├── lib/                   # 前端纯逻辑（markdown 渲染与渲染后增强、高亮补充语言表、URL 路由解析、博客布局与导航菜单、博客文章块部件、博客皮肤与主题库、博客自定义脚本解析、博客 SEO/预渲染字符串、博客列表筛选与标签云聚合、评论线程与待审本地合并、通知渠道与凭据掩码、剪藏、缩略图、敏感扫描、图片调宽、xmind、fm 工具等）
│   ├── hooks/
