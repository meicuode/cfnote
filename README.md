...
- **文件管理多选与批量操作**：列表按 Windows 资源管理器的方式选择——**单击选中、Ctrl/⌘ 点选、Shift 连选、Ctrl+Shift 追加一段、Ctrl/⌘+A 全选、Esc 清空、双击打开**；选中后一次性**移动 / 复制 / 删除 / 复制链接**，也可以直接**把选中的文件拖到左侧目录树**上移动（含只在拖拽时出现的「拖到此处移出文件夹」落点）。**底部状态栏**没选中时显示当前目录合计（文件数 + 总大小），选中后显示「已选 N 个 · 共 X」。行首复选框默认不显示，**触屏设备自动显示**（那里没有修饰键），可在「设置 → 文件管理」改成总是显示/从不显示（存本机，文件管理右上角齿轮可直达）。批量走一个 `POST /api/fm/files/batch`，不是前端循环打 N 次请求。删除时若有文件仍被笔记引用，服务端先回一份名单让你确认；复制会真的在 R2 多存一份（副本是新链接、不继承分享），因此限一次 ≤20 个、单个 ≤10MB，超出的跳过并列出原因
...
│   │   ├── FileManager.tsx    # 文件管理右侧列表（搜索/预览/分享/清理/多选批量操作/拖拽到目录，内联工作区视图）
