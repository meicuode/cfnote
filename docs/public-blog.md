...
- 主题库(P12.7):内置那 5 个「预设」不是主题,只是一键改主色与顶栏色的按钮(`applyPreset` 明确保留排版设置),所以调顺手的整套配置此前没地方存、也没法存两套换着看。现在加 `settings.blog_skin_library`:一个 `{id, name, skin}[]` 数组,支持新建 / 套用 / 更新(把当前配置写回某套)/ 重命名 / 删除 / 导出 / 导入。**刻意与 `blog_skin` 分成两个键、两个模块**——`blog_skin` 是「当前生效的那一套」,要随博客响应下发、worker 侧也要用;主题库只有管理端读。这样存多少套主题都不增加公开路径的 D1 查询与响应体积,`blogThemes.ts` 也不会被打进 worker 包。上限 30 套只是防止 settings 那一行无限膨胀。
- 主题的导入导出(P12.7):导出的是一份配置 JSON(`{app, version, name, skin}`),**不是 WordPress 那种含模板的主题包**——我们的「主题」本来就只有配色、排版、额外 CSS 这几项值,叫「主题配置」更诚实。导入时一律把 `skin` 过一遍 `parseBlogSkin`(而不是信任外来 blob 的任何一项),额外 CSS 再过 `sanitizeCss`。另外剥掉 `@import`:CSS 不能执行 JS,但 `@import url(https://…)` 一定会发请求,把访客的 IP 与 Referer 送给第三方——自己写的 CSS 里这么干是本人的选择,从别人那里导入的就不是了。外部 `url()` 则保留但列出来让人过一眼,因为确实有人用 CDN 上的背景图,一刀切删掉会让导入的主题静默走样。
- id 与重名(P12.7):主题 id 取现有 `t<数字>` 的最大值 +1,不用时间戳或随机数——那样单测就得跟时钟打交道,这里完全没必要引入不确定性。重名自动加序号(「墨绿」→「墨绿 2」)而不是拒绝保存;但**打字过程中不去重**,否则输到与已有主题同名的那一刻就会被自动改名,只在失焦时归一。
- 主题 JS 不做(P12.7 的明确取舍):博客页与管理端、`/api/*` 在**同一个源**上,博客页里跑的任意 JS 可以带着登录 cookie 去 `fetch('/api/articles')` 把整个知识库读走,而博主每次推送后都要登录着去博客页验证——正好是这个攻击成立的条件。CSS 出问题最多是页面难看,JS 出问题是账号沦陷,不在一个量级。真要支持主题 JS,前置条件是**把博客放到独立主机名**(如 `blog.example.com` 指向同一个 Worker,管理端只在主域),那时博客 JS 才是跨源的、碰不到 `/api/*` 的 cookie。若只是要统计代码,应当开一个只收脚本 URL 的窄口子,而不是自由文本框。
- 文章块部件(P12.8):P12.1–12.4 的模块系统管的是**文章周围**(顶部/左栏/右栏/底部四个槽位),文章块本身一直写死:面包屑 → 标题 → 元信息行 → 正文 → 「· 完 ·」→ 评论。去不掉「来源:CFNote 笔记」、藏不了浏览数、Tags 挪不到正文下面——这是 WordPress 的 `single.php` 能干而我们干不了的事。现在拆成 8 个可排序、可开关的部件(面包屑 / 标题 / 元信息 / 正文 / 标签行 / 结束标记 / 版权声明 / 评论区),存 `blog_layout.article`,跟着布局一起下发,不新增 settings 键、不多一趟 D1。默认部件表逐项等于改造前(新增的标签行与版权声明默认关闭)。
- 为什么不做模板语言(P12.8):那会让「服务端预渲染」与「客户端 React」变成两个模板引擎,必然走样——与 P12.6 拒绝预渲染那 12 种模块、P12.4 拒绝「仿真画布」是同一个论证。部件是一份声明式的有限清单,两边各自照着它渲染,预渲染那侧只需改 `articleBlockHtml` 一个函数。**成员是固定的**:不能增删,只能排序与开关。这样「详情页由这几块组成」是个恒定的心智模型,也省掉了「配置里没有正文怎么办」这类边界;解析时未知类型丢弃、重复只留第一个、缺失的补默认值。
- 两个锁定的部件(P12.8):正文不必解释;评论区之所以只能移动不能停用,是因为它的开关已经在「设置 → 评论」里了,而那个开关同时会让 `POST /api/blog/comments` 返回 403。若这里再给一个「隐藏评论区」,就成了同一件事两个开关,迟早出现「这里关了那里还能提交」的困惑。
- 预渲染必须跟着走(P12.8):这是 P12.6 埋下的必然联动——`articleBlockHtml` 照**同一份 `layout.article`** 产出 HTML,否则抓取器看到的结构会和读者看到的不一致。评论区永远不预渲染(变化频率远高于文章,会毁掉 60 秒边缘缓存);版权声明的 Markdown 在 worker 侧用 `marked` 渲染好再传进来,`blogSeo.ts` 保持零依赖。每个部件保留它原来的上边距类,所以默认配置渲染出来与改造前逐项一致;换了顺序就是各自带着自己的间距。
- 版权声明按 Markdown 渲染(P12.8):内容由博主自己撰写,与自定义 Markdown 模块同等信任、同一条渲染路径(仓库无 HTML 消毒库,故不开裸 HTML 入口)。客户端那份同样要 `useMemo` 缓存引用,理由见上面那条 P11.8.1。
