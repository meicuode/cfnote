...
**这是活文档**,随附件/文件管理的改动一起更新;**覆盖到 P14.2**(2026-08-01)。逐批的决策与取舍见 [`../DESIGN.md`](../DESIGN.md) §10,文档分工见[文档地图](README.md)。

**当前状态**:P8 四期全部完成(P8.1 数据层/间接链接/访问分级/发布弹窗附件清单、P8.2 文件管理页、P8.2.1 私密文件夹与分享链接、P8.3 编辑器文件库选择器);此后经 P11.5/P11.6 内联化与侧栏二级菜单、P13.3/P13.7/P13.8 多选与右键菜单、P14.1 回收站语义、P14.2 移入私密前的公开引用预检。与 [evernote-gap.md](evernote-gap.md) 的「附件公开治理(A1+A2)」共用地基,**本文档为准**(A2 的 LIKE 查询方案已由关联索引表取代)。
...
**软删除(进回收站)完全不碰附件**:`article_files` 行保留、R2 一个字节不动。理由不是图省事——附件是**多对多**的:同一张图可能被三篇笔记引用而只有一篇进了回收站。「附件跟着进回收站」只有两种可能含义,两种都错:让文件在文件管理里消失(另外两篇活着的笔记当场变死链——可逆的回收站反而把还活着的东西弄坏了),或者只打一个不影响任何行为的标记(纯装饰)。

正确语义是:**附件的生死只看「还有没有活着的引用」,而回收站里的笔记算活着的引用**(它可能被恢复)。

**彻底删除时**(单篇 purge / 清空回收站 / 30 天 cron / 笔记本 purge,同一条管线 `orphanKeysAfterPurge`)按三档判定:

| 情况 | 结果 |
|---|---|
| 还被别的文章引用(**含回收站里的**) | 留 |
| 引用归零,但 `folder_id` 非空 | 留,落到「未引用」视图等你处置 |
| 引用归零,且从没归过文件夹 | 连同 xmind 边车一起清 R2,并删 `files` 登记 |

第二档是 P8 定的:**一旦你在文件管理里把文件归进文件夹,它就从「某篇笔记的附身之物」变成「你主动收藏的资产」**,删笔记不该带走它。这也正是「未引用」视图存在的理由。

**清空前有只读预检** `GET /api/articles/trash/impact`,与真正的清理**共用同一个判定函数**,所以确认框里那句「将清理 N 个附件(共 X)」不会和实际结果对不上。预检只在点了「清空」、确认框弹出时打一次——不占首屏。

`orphanKeysAfterPurge` 用集合查询而不是逐 key 循环:清空一个 200 篇 × 5 附件的回收站,老写法是约 3000 次串行 D1 往返,很容易撞上 Worker 的挂钟上限;占位符按 200 分片避免撞 SQLite 的绑定参数上限。

**引用清单包含回收站里的笔记**(带 🗑 徽标)。此前 `ref_count` 算它们、引用清单弹窗却把它们滤掉,于是会出现「显示 1 篇笔记引用,点开却说没有引用」——而它们恰恰就是这个文件不被自动清理的原因,更该看得见。
...
**移入私密文件夹前的公开引用预检(P14.2)**:上面那条「一票否决」有个此前没人提醒的后果——把一张**正被已公开笔记引用**的图拖进私密文件夹,博客上那张图当场变裂图,而这个动作原本是静默成功的。更难发现的是:你自己登录着,看什么都正常,只有访客看到裂图。

所以三条移动路径(批量移动 / 单个移动 / 目录整体移动)在落点位于私密子树时先做一次只读预检,返回 `{needs_force: true, public_refs: [{id, name, articles: [...]}]}`,前端摊开「这些已发布页面里的图会失效」再让人决定,确认后带 `force: true` 重发。与批量删除那条「仍被笔记引用需确认」是同一套形状,只是名单不同。两处细节:

- **已经在私密子树里的文件不算**——私密目录之间互相搬动改变不了任何可见性,再问一遍就是噪音。
- 目录整体移动时受影响的是**它整个子树**里的文件(`fileIdsUnderFolder` 先在内存里 BFS 出子树,再按 200 分片查文件),否则埋在两层目录下的图会漏掉。

判定只看「公开且非私有、且未进回收站」的引用——未公开的笔记访客本来就看不见,拦它没有意义。
...
- **P13.3 批量操作与拖拽** ✅(2026-07-30):多选 + 一个 `POST /api/fm/files/batch` 打完移动/删除/复制(不在前端循环打 N 次请求——移动 20 个文件就是 20 次计费请求);复制是唯一有真实成本的(R2 的 Workers binding 没有服务端 copy,必须 get 再 put、字节流经 Worker,故限一次 ≤20 个、单个 ≤10MB);行 `draggable` 拖到侧栏目录节点落地。
- **P13.7 资源管理器式多选 + 底部状态栏** ✅(2026-07-31):单击独选 / Ctrl 点选 / Shift 连选 / Ctrl+Shift 追加 / Ctrl+A / Esc / 双击打开,判定在 `src/lib/fmUtils.nextSelection`(纯函数 + 单测:选错行等于批量删掉没打算删的)。行首复选框降级为可选项(`auto` 仅触屏 / `on` / `off`),存 **localStorage** 而非 settings 表(每台设备各自的显示偏好,且放服务端会让每次打开文件管理多打一次 `/api/settings`);触屏判定用 `matchMedia('(pointer: coarse)')` 而不是窗口宽度。「拖到此处移出文件夹」那块落点改为**只在拖拽进行中出现**。
- **P13.8 行右键菜单** ✅(2026-07-31):每一项都调已有的处理函数,零新增端点。右键未选中的行先把选择替换成它、右键选中集里的行则整段保持——菜单永远作用在 `sel` 上,不必再引入一个「右键目标」的平行状态。落点计算抽成 `fmUtils.menuPosition()`(纯函数 + 单测:错了的表现是「菜单一半在屏幕外、点不到删除」)。关闭用一块透明遮罩而不是全局捕获监听(`mousedown` 与 `click` 是两个独立事件)。侧栏文件夹树不加。
- **P14.1 回收站语义** ✅(2026-08-01):软删除完全不碰附件;彻底删除时三档判定;清空前有只读预检。见上文「附件与回收站」。
- **P14.2 移入私密前的公开引用预检** ✅(2026-08-01):三条移动路径先返回会失效的公开文章清单,确认后才带 `force`。见上文「私密文件夹与分享」。
