...
> 它有两部分,新鲜度不一样:
>
> - **第三节的「与 Evernote 的功能差异」对标表是活的** —— 「还差什么、哪些明确不做」以那张表为准,每次评估后更新。
> - **下面那一长串批次记录冻结在 P12.5**(2026-07-28)。**P12.6 起的批次记录统一在 [`../DESIGN.md`](../DESIGN.md) §10**,不再往这里追加。
>
> 为什么不补:一批往往横跨附件、博客、编辑器三处,拆开写在三份文档里等于维护三份变更日志,过几个月谁也说不清哪份最新。见 [文档地图](README.md)。
>
> 第一、二节(附件与「公开」的关系、A1+A2 方案)是 P8 立项时的现状盘点与选型论证,**方案已全部落地**,留作决策来由;附件体系当前的样子看 [file-manager.md](file-manager.md)。
...
- **回收站**:`articles.deleted_at` 软删除;删除即清向量/分块并取消公开与置顶(搜索、AI、博客立即不可见),附件引用行保留防误清;侧栏「回收站」虚拟视图支持恢复(重建向量)/彻底删除/清空;30 天自动清理(打开回收站懒执行 + cron 兜底);回收站笔记打开为只读(横幅提示)。**注意:删除整本笔记本仍是彻底删除不进回收站**(articles 对 notebooks 有 ON DELETE CASCADE 外键,改约束需重建表,弹窗文案已注明)。 ⟵ **已被 P14.1 推翻**:不必改那个外键——只要永远不 DELETE notebooks 那一行,CASCADE 就永远不会触发。现在删笔记本也是软删除,见 `DESIGN.md` §10 与 [file-manager.md](file-manager.md) 的「附件与回收站」一节。
...
**P12.5(2026-07-28)✅**:博客皮肤——配色预设、排版旋钮、额外 CSS。

- **旋钮刻意只有 8 个**:主色 / 顶栏色 / 圆角 / 正文字号 / 容器宽度 / 字体 / 列表样式 / 额外 CSS。不把十几个 `--blog-*` 全放出来的理由是:中性色(卡片、边框、正文灰)是成对调过的明暗值,单独改一个很容易配出读不了的组合;而主色 + 顶栏色一换,logo、链接、热榜序号、按钮、顶栏页脚全跟着变,观感上就是另一套主题。要精细控制交给「额外 CSS」——它能覆盖任意 `--blog-*` 变量,等价 WordPress 的「额外 CSS」。
- **5 套预设**(IT之家红/墨绿/深蓝/酒紫/石墨)只改这两个颜色,排版设置保留;默认皮肤逐项等于改造前的取值(圆角 8px = `rounded-lg`,容器 1400px = 原 `max-w-[1400px]`),不配置则页面零变化。
- **悬浮色可留空**表示跟随主色自动提亮(`lighten(accent, 0.22)`),省得让人填两个颜色;这也意味着解析时空串是**合法值**,不能走 `normalizeHex` 的回落——这是这批唯一一个容易写错的地方。
- **变量以内联 style 挂在博客根节点**,优先级高于 index.css 里的同名默认值,而默认值仍留着做兜底(接口没返回皮肤时页面照常)。
- **额外 CSS 的安全性**:走 React 的 `<style>` 文本节点是 DOM 赋值、不经 HTML 解析器,本就不存在 `</style>` 逃逸;`sanitizeCss` 再做一道过滤与 8000 字符上限,是为将来万一改成拼字符串输出留的保险。内容由博主自己写,与自定义 Markdown 模块同等信任。
- **容器宽度可调的连带修改**:`contentWidth(page, containerMax)` 要接受宽度参数,博客页的 `max-w-[1400px]` 与目录浮层那两处 `calc((100vw-1400px)/2-…)` 全部改走 `--blog-max`,否则调宽容器后目录浮层会算错位置。
- **「纯文字列表」不只是观感选项**:不出缩略图,每篇省掉一次图片请求,对免费额度是实打实的。
- 皮肤存 `settings.blog_skin`,与 `blog_layout` 一次 `IN` 查询取回、随同一次响应下发,**不多一趟 D1、不多一次请求**;无 schema 改动;tsc 零报错,326 用例全绿(+23)。
...
> **A1 与 A2 都已在 P8 落地**(下面是当初的选型论证,留作决策来由)。实际实现比这里写的更进一步:免登录判定不是 `content LIKE`,而是 `article_files` 引用索引反查(`instr` 只作索引缺行时的兜底),另加了「我的私密文件夹」这层结构化私密。当前形态见 [file-manager.md](file-manager.md)。
...
> **这一节是活的**,每次评估后更新(最近一次:2026-08-01 / P14.3)。

### 已对齐或超出

笔记本/笔记管理、富文本+源码+预览编辑(标准 Markdown 存储)、附件(任意文件,XMind 在线编辑超出)、混合搜索(语义部分超出)、AI 问答与引用定位(对标 Evernote AI)、公开发布(**博客整站 + 可配置布局/皮肤/主题库/访客评论**,远超单篇分享)、全量导出/批量导入(**外加自动备份到 R2**)、深色模式、用量统计。

### 对标清单与状态

| # | 功能 | Evernote 对应 | 状态 | 落在哪 |
|---|------|--------------|------|--------|
| 1 | 附件公开治理 | (安全基座) | ✅ P8 | A1 发布弹窗清单 + A2 访问分级,见本文第二节与 [file-manager.md](file-manager.md) |
| 2 | 回收站(软删除) | 回收站 | ✅ P9 / **P14.1 扩到笔记本** | 笔记本删除也走软删除,附件按三档判定,清空前有只读预检 |
| 3 | 标签系统 | 标签 | ✅ P9 | `articles.tags` JSON 列 + `json_each`(**没有建 tags/article_tags 两表**,单用户量级不值) |
| 4 | 置顶/快捷方式 | 快捷方式 | ✅ P9 | `articles.pinned` |
| 5 | 网页剪藏 | Web Clipper(招牌) | ✅ P9.4 | bookmarklet + `/clip`,零后端改动 |
| 6 | 笔记间链接 + 反链 | 笔记链接 | ✅ P9.2 | 反链超出 Evernote |
| 7 | 任务清单勾选 | 任务 | ✅ P9.2 | GFM `- [ ]` 预览内可点,回写源文 |
| 8 | 私密分享链接 | 共享链接 | ✅ P9.3 | `/blog/share/<token>`,七档有效期 |
| 9 | 笔记模板 | 模板 | ✅ P9.3 | 约定名为「模板」的笔记本 |
| 10 | 版本历史 | 历史(付费) | ✅ P10.1 | `article_versions`,同小时合并 + 保留策略 |
| 11 | Email 收集箱 | Email 转发进笔记 | ❌ **不做** | 要自有域名 + Email Routing,而剪藏(#5)已覆盖「把外面的东西弄进来」这个需求的绝大部分 |
| 12 | 提醒 | 提醒 | ✅ P10.2 / P10.3 **超出** | 应用内铃铛 + 六种渠道推送到手机,见 [notifications.md](notifications.md) |
| 13 | OCR / 附件内搜索 | 图片文字、PDF 搜索(招牌) | ❌ **不做** | 每张图一次视觉模型调用,免费额度扛不住;而个人库里图片多是截图配图,不是待检索的原始资料 |
| 14 | PWA 离线 / 移动完整版 | 客户端离线 | 🟡 **只做了「只看」**(P15.1) | 管理端窄屏可用:侧栏→列表→正文的返回栈、手机默认预览、触屏常驻操作。**离线/PWA/Service Worker/冲突合并明确不做**——那是另一个量级的工程,且单用户场景下「断网时改笔记」的价值远低于它引入的冲突面 |
| 15 | 笔记本分组(Stack) | 笔记本组 | ✅ **做了,而且是真树**(P16.1) | ~~平铺 + 标签 + 搜索已经够用;加一层树只会让「这篇该放哪」多一个纠结点~~ ——**这条被 P16.1 推翻**。当时的理由针对的是「自己从头建层级」,而真正的场景是**把已经按目录整理好的本地知识库迁进来**:结构是现成的,不存在「该放哪」的纠结,反倒是不能保留结构就没法迁。Evernote 自己只有 Stack>Notebook 两层且笔记本上限 250,这里做的是 `notebooks.parent_id` 的无限层级 |

### 超出 Evernote 的部分(它没有的)

博客整站(列表/详情/RSS/sitemap/SEO 预渲染)、可配置页面布局与皮肤主题库、访客评论体系(审核/嵌套/极小 Markdown 子集)、语义检索与 AI 问答、Mermaid/KaTeX 渲染、文件管理页(资源管理器式多选/右键/拖拽)、附件分享链接、自动备份到 R2、用量仪表盘。
...
原计划(P8 基座 / P9 采集互联 / P10 进阶)基本按序走完,之后的批次已远超这份清单的范围:

- **P8** 附件治理与文件管理体系 → 见 [file-manager.md](file-manager.md)
- **P9** 回收站、标签、置顶、剪藏、笔记链接、任务勾选、私密分享、模板
- **P10** 版本历史、提醒与多渠道推送、标签区重构、代码高亮与 KaTeX、URL 路由
- **P11** 博客管理与访客评论、Mermaid、内联化与二级菜单、章节目录、评论来源
- **P12** 博客模块化布局、分页与服务端筛选、皮肤与主题库、SEO 预渲染、备份完整性、自定义脚本
- **P13** 单页、文件管理多选/右键/拖拽、博客取数层、Worker e2e 测试、评论 Markdown 子集与表情
- **P14** 数据安全:笔记本软删除与附件三档判定、自动备份到 R2、私密文件夹的公开引用预检

**逐批的决策与取舍在 [`../DESIGN.md`](../DESIGN.md) §10**,那里才是完整的。
