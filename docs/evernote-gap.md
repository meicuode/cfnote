...
- ~~**归档页**~~:移到下面「不做」了(P16.4 期间结的账)。

**这一栏现在是空的**——博客侧想做的都做完了。剩下的待办都在管理端,见 `DESIGN.md` §10 末尾。

### 不做(理由)

- **归档页**(按年月的文章索引页):列表已有分页 + 标签筛选 + 全文搜索,再加一个按时间的入口,是给「不知道自己要找什么」的访客准备的——而个人博客的访客几乎都是**带着具体问题从搜索引擎进来**的。真正按时间回看的是我自己,那在管理端,不在博客。
- 多人协作/共享编辑:单用户定位。
